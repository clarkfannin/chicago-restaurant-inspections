...
def prepare_for_sheets(df, table_name):
...
for csv_name in CSV_FILES:
    ...
    data = prepare_for_sheets(df, table_name)
    ws.update(data)
    ...
    for col in date_cols:
        if col in df.columns:
            col_idx = df.columns.get_loc(col) + 1
            col_letter = chr(64 + col_idx) if col_idx <= 26 else chr(64 + col_idx // 26) + chr(64 + col_idx % 26)
            ws.format(f'{col_letter}2:{col_letter}', {"numberFormat": {"type": "DATE", "pattern": "yyyy-mm-dd"}})
